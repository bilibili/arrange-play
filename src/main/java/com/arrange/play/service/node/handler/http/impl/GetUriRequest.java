...
@Slf4j
@Component
public class GetUriRequest extends HttpRequest {
  ...
  public Response request(HttpRequestDTO httpRequestDTO) {
    ...
    return HttpRequestUtil.getRequest(url, TransUtils.mapToUri.join(
        JSON.parseObject(httpRequestDTO.getHttpRequestModule().getModuleContent())),
        TransUtils.jsonToMap(JSONObject.parseObject(httpRequestDTO.getHttpRequestModule().getHeader())));
